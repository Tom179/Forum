...
// getEncoder 设置日志存储格式
func getEncoder() zapcore.Encoder { //Encoder编码器负责将日志消息和结构化数据格式化为特定的输出格式，例如 JSON、文本格式（Console）
...
// getLogWriter 日志记录介质。GoWeb 中使用了两种介质，os.Stdout 和文件
func getLogWriter(filename string, maxSize, maxBackup, maxAge int, compress bool, logType string) zapcore.WriteSyncer {

	// 如果配置了按照日期记录日志文件
	if logType == "daily" {
		logname := time.Now().Format("2006-01-02.log") //获取当前时间
